...
Born2beroot에서 우리는 **Linux Server 한 대를 만들고 관리하는 방법**을 배웠습니다.
...
이 문제를 해결하기 위해 **Container와 Docker**를 사용합니다.
...
Container는 **Host의 Kernel을 공유하면서 격리된 환경에서 실행되는 Process**라고 이해하면 좋습니다.
...
그래서 Container를 **만들고, 실행하고, 연결하고, 저장하고, 삭제하는 작업을 편하게 관리하는 Platform**이 필요합니다.
...
의 `docker`는 Client입니다.

실제 Container와 Image 등을 관리하는 핵심 Process는 Docker daemon인 `dockerd`입니다.
...
Born2beroot에서 배운 `systemctl`이 여기서 그대로 다시 등장합니다.
...
이 실행 환경을 Packaging한 것이 **Image**입니다.
...
그 역할을 하는 것이 **Container Registry**입니다.
...
을 사람이 기억해서 반복하는 대신, **Image를 만드는 과정을 File로 기록**할 수 있습니다.
...
`apt-get install -y`의 `-y`는 설치 질문에 자동으로 yes를 선택합니다.
...
**중요:** `EXPOSE`만 쓴다고 Host에 Port가 자동으로 공개되는 것은 아닙니다.

실제 Host Port Publishing은 `docker run -p` 또는 Compose의 `ports`에서 설정합니다.
...
Container는 보통 **주 Process가 살아 있는 동안 실행 상태를 유지**합니다.
...
를 실행하려면 `conf/nginx.conf`가 Build Context 안에 있어야 합니다.
...
Container ID 대신 `web`이라는 이름으로 조작할 수 있습니다.
...
Browser에서 Host의 `8080`으로 접근하면 Container의 `80`으로 전달됩니다.
...
그래서 `-it`를 같이 사용하면 Container 내부에서 Terminal을 사용하는 느낌으로 작업할 수 있습니다.
...
따라서 **Application의 생명주기와 Data의 생명주기를 분리**해야 합니다.

이 문제를 해결하는 것이 **Volume**입니다.
...
**현재 WordPress Container 자기 자신**입니다.
...
즉 Inception은 **세 개의 독립된 Service를 Network와 Volume으로 하나의 Application처럼 연결하는 프로젝트**라고 볼 수 있습니다.
...
`wordpress`는 Docker Network의 Service 이름이고 `9000`은 PHP-FPM이 Listen하는 Port입니다.
...
이런 값을 **Environment Variable**로 전달할 수 있습니다.
...
Compose에서는 반복되는 값을 `.env`에 둘 수 있습니다.
...
주의할 점은 **Environment Variable과 Secret은 완전히 같은 개념이 아니라는 것**입니다.
...
> `depends_on`으로 Container가 먼저 시작되었다고 해서 Application이 실제 요청을 받을 준비까지 끝났다는 뜻은 아닙니다.
...
**주의:** `-v`는 Persistent Data를 담은 Volume까지 제거할 수 있습니다.
...
`nginx -t`는 NGINX Configuration Syntax를 검사합니다.
...
Compose가 만든 Network 이름은 Project Prefix가 붙을 수 있으므로 실제 이름은 `docker network ls`로 확인합니다.
...
Command Line에서는 `curl`이 매우 유용합니다.
...
`-k`는 TLS Certificate 검증을 생략하므로 **Debugging/학습용으로만** 이해해야 합니다.
...
처음에는 Output이 매우 길기 때문에 `docker inspect`는:
...
| 목적 | 명령어 |
|---|---|
| Docker 상태 | `docker info` |
| Image 목록 | `docker image ls` |
| Image Build | `docker build -t name .` |
| Image Pull | `docker pull image` |
| Container 실행 | `docker run image` |
| 실행 Container | `docker ps` |
| 모든 Container | `docker ps -a` |
| Container 중지 | `docker stop name` |
| Container 삭제 | `docker rm name` |
| 내부 Shell | `docker exec -it name sh` |
| Logs | `docker logs name` |
| Network 목록 | `docker network ls` |
| Network 상세 | `docker network inspect name` |
| Volume 목록 | `docker volume ls` |
| Volume 상세 | `docker volume inspect name` |
| Object 상세 | `docker inspect name` |
| Compose Build | `docker compose build` |
| Compose 실행 | `docker compose up -d` |
| Build + 실행 | `docker compose up -d --build` |
| Compose 상태 | `docker compose ps` |
| Compose Logs | `docker compose logs -f` |
| Service Shell | `docker compose exec service sh` |
| Compose 종료 | `docker compose down` |
| NGINX Config 검사 | `nginx -t` |
| Listening Port | `ss -tlnp` |
| HTTP 테스트 | `curl -v URL` |
...
Inception of Things에서 **Orchestration**으로 관심 범위가 확장됩니다.
